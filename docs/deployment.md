...
| Build | `npm ci --include=dev && npm run build:calculator && npm run build:web` | `npm ci --include=dev && npm run build:calculator && npm run build:api` |
...
3. Copy the API's actual HTTPS URL assigned by Render. Set the frontend environment variable `NEXT_PUBLIC_API_BASE_URL` to that URL, without a trailing slash.
...
The initial Blueprint intentionally leaves `NEXT_PUBLIC_API_BASE_URL` empty and allows only the local development origin for API browser requests. This prevents a guessed hostname from being represented as a working production integration. Configure the actual URLs in the steps above.
