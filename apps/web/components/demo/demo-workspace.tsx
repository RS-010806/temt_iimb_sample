...
import { DEFAULT_FILTERS, MODE_COLORS, MODE_LABELS, QUICK_SCENARIO_NAME, REQUIRED_CSV_COLUMNS, SECTORS, backendErrorMessage, clearQuickQuery, filteredAnalysis, formatNumber, legKey, makeQuickScenario, makeScenario, matchesFilters, normalizeCSVRows, parseDemoQuery, profileFor, shortNumber, type Company, type DemoLeg, type Filters, type QuickEstimate, type Sector } from "../../lib/demo-data";
...
export function DemoWorkspace() {
  ...
  const companyName = quickEstimate ? QUICK_SCENARIO_NAME : company?.name || SECTORS[sector].company;
  ...
  const updateUrl = (nextSector: Sector, nextCompany: Company | null) => { const url = new URL(window.location.href); url.search = clearQuickQuery(url.search).toString(); url.searchParams.set("sector", nextSector); if (nextCompany) url.searchParams.set("company", nextCompany.symbol); else url.searchParams.delete("company"); window.history.replaceState({}, "", url); };

  const loadScenario = useCallback((nextSector: Sector, nextCompany: Company | null = null, persistUrl = true) => {
    invalidateBackend(); setQuickEstimate(null); setSector(nextSector); setRawRows(makeScenario(nextSector)); setCompany(nextCompany); setCompanySearch(""); setCompanyOpen(false); setFilters({ ...DEFAULT_FILTERS }); setSynthetic(true); setPage(0); setImportError([]); setImportNotice(""); setRailShare(30);
  ...
  useEffect(() => {
    ...
    if (query.quick) { invalidateBackend(); setQuickEstimate(query.quick); setRawRows(makeQuickScenario(query.quick)); setCompany(null); setSynthetic(true); setFilters({ ...DEFAULT_FILTERS }); }
    else if (initialSector !== "fmcg") loadScenario(initialSector, null, false);
    applyUrlInputs();
    const initializedGeneration = generationRef.current;
    fetch("/data/nifty500.json", { signal: controller.signal }).then(response => { if (!response.ok) throw new Error("Company directory unavailable"); return response.json(); }).then((data: Company[]) => {
      ...
      if (match && !query.quick && generationRef.current === initializedGeneration) { loadScenario(match.sector !== "general" ? match.sector : initialSector, match, false); applyUrlInputs(); }
  ...
  }, [invalidateBackend, loadScenario]);
  ...
  const importCSV = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      invalidateBackend(); setQuickEstimate(null); setRawRows(imported); setSynthetic(false); setCompany(null); setFilters({ mode: "all", subsidiary: "all", from: "", to: "" }); setPage(0); setImportNotice(`${formatNumber(imported.length)} validated legs imported from ${file.name}. Data stays in your browser until you run backend analysis.`); const url = new URL(window.location.href); url.search = clearQuickQuery(url.search).toString(); url.searchParams.delete("company"); window.history.replaceState({}, "", url); announce("CSV imported. Local analytics are ready.");
  ...
  const doExport = async (type: "csv" | "pdf" | "powerbi") => {
    ...
    try { const { exportCSV, exportPDF, exportPowerBI } = await import("../../lib/demo-export"); const context = { analysis: view, rawRows, name: synthetic ? quickEstimate ? QUICK_SCENARIO_NAME : `Illustrative ${companyName} scenario` : "Imported freight data", filters, synthetic, processing: processing === "backend" ? "backend" : "local" }; await ({ csv: exportCSV, pdf: exportPDF, powerbi: exportPowerBI }[type])(context); announce(`${type === "powerbi" ? "Power BI data pack" : type.toUpperCase()} downloaded with the current filters.`); } catch { announce("Export could not be generated. Please try again."); } finally { setExporting(false); }
  };

  const sampleDownload = async () => { try { const { downloadSample } = await import("../../lib/demo-export"); await downloadSample(quickEstimate ? makeQuickScenario(quickEstimate) : makeScenario(sector)); announce("Sample CSV downloaded."); } catch { announce("Sample download failed. Please try again."); } };
  ...
  return <div className={styles.workspace}>
    ...
    <main className={styles.main} id="main">
      ...
      <div className={styles.content}>
        ...
        {slowRequest && <div className={styles.notice} role="status"><Loader2 size={15} className={styles.spinner}/><p>The backend is taking longer than expected. Your local preview is ready.<button className={styles.textButton} onClick={continueLocally}>Process in browser</button></p></div>}
        ...
        <section className={styles.metrics} aria-label="Filtered freight metrics"><Metric label="Total freight emissions" value={formatNumber(view.totals.emissionsKg / 1000, 2)} unit="tCO₂e" foot="Calculated from the selected legs" icon={<Leaf size={16}/>} primary/><Metric label="Shipments in view" value={formatNumber(view.totals.shipmentCount)} foot={`${formatNumber(view.totals.legCount)} ${view.totals.legCount === 1 ? 'leg' : 'legs'} across ${view.bySubsidiary.length} ${view.bySubsidiary.length === 1 ? 'business unit' : 'business units'}`} icon={<Package size={16}/>}/><Metric label="Freight activity" value={shortNumber(view.totals.tonneKm)} unit="tkm" foot="Cargo mass × distance travelled" icon={<Route size={16}/>}/><Metric label="Emission intensity" value={formatNumber(intensity, 1)} unit="g / tkm" foot="CO₂e per tonne-kilometre" icon={<Activity size={16}/>}/></section>

        <div className={styles.analysisLayout}><div className={styles.analyticsMain} id="analytics">
