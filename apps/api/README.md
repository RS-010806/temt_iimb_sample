...
`PORT` defaults to 3001. `TRUST_PROXY_HOPS` defaults to 0; set it to the verified number of controlled reverse proxies (normally 1 for a directly deployed Render service). The service limits `/api` traffic to 60 requests per minute per IP, excluding health checks and CORS preflights, and returns 429 when exceeded. This limiter is process-local and is not a distributed quota. The frontend should retain its local fallback and expose which calculation path was used.
